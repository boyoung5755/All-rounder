	...
	$.ajax({
		...
		success: function(memoList) {
			...
			$.each(memoList, function(index, memo) {
				...
				let $newMemo = $('<div class="memo" id="' + memo.memoNo + '" style="background-image: url(' + memo.memoBg + ');z-index:' + memo.memoZindex + '; left: ' + left + 'px; top: ' + top + 'px;"><div class="btnClose">&times;</div><div class="btnCheck" style="color: ' + color + ';" data-check="' + memo.memoBkmkYn + '">&#9733;</div><textarea class="txtMemo">' + memo.memoCn + '</textarea></div>');
...
function insertOpenModal() {
	...
	$('#addMemo').click(function() {
		...
		let memo = content.value;
		 ...
		 // 즐겨찾기 여부
		let check = document.getElementById('memoCheck');
