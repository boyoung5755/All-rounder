...
//	@Transactional
	...
	public ServiceResult createJob(PjobVO jVO) {
		...
		if(jVO.getJobuSn() != null && jVO.getJobStcd() == null) { //하위일감인데  상태를 클릭하지 않았으면
			jVO.setJobStcd("1"); //자동으로 일감을 진행상태로
		}
	...
	@Override
	public List<PjobVO> retrieveReq(String loginEmpCd, String proSn ) {
		return dao.selectMyReq(loginEmpCd,proSn);
	}
